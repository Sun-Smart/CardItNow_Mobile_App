...
class AuthCon extends GetxController with BaseController {
  ...
  void loginAPI(email, password, bool ischecked_checkbox) async {
    ...
    var result = GetStorage().write("save_token", data["token"].toString());
    print(result);
    ...
    var response = await BaseClient()
        .post(
            API().uploadProcessDocument +
                "?email=$email&documenttype=$choosedDocId&document=$scandocs&documentid=$docid&selfi=$uploadimg",
            body)
        .catchError(handleError);
    if (response == null) return;
    var data = json.decode(response);
...
//get method
  ...
  void taxDetailsgetApi() async {
    var responce = await BaseClient().get(API().taxDetailsGetApiData).catchError(handleError);
  ...
  void ocrdocument() async {
    var body = {
      "livestockphoto": scandocs,
    };
    var response = await BaseClient()
        .post(API().processocr, body)
        .catchError(handleError);
